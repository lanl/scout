...
void init_mesh(unsigned width, unsigned height, unsigned depth, unsigned cell_size) {
    ...
    for(unsigned y = 0; y < mesh_height; y++) {
      for(unsigned x = 0; x < mesh_width; x++) {  
        for(unsigned face = 0; face < 6; face++) {
          face_colors[i]   = float(z) / (mesh_depth-1);
          face_colors[i+1] = float(y) / (mesh_height-1);
          face_colors[i+2] = float(x) / (mesh_width-1);
          printf("color[%d,%d,%d] = (%f, %f, %f)\n", i, i+1, i+2,
                 face_colors[i], face_colors[i+1], face_colors[i+2]);
          face_count++;
          i += 3;                 
        }
      }
  ...
  for(unsigned z = 0; z <= mesh_depth; z++) {
    for(unsigned y = 0; y < mesh_height; y++) {
      for(unsigned x = 0; x < mesh_width; x++) {
        unsigned i0, i1;
        i0 = z * (mesh_width+1) * (mesh_height+1) + y * (mesh_width+1) + x;
        i1 = z * (mesh_width+1) * (mesh_height+1) + y * (mesh_width+1) + x + 1;
        faces[i]   = i1;
        faces[i+1] = i0;
        faces[i+2] = i0 + 3;
        faces[i+3] = i1 + 3;
