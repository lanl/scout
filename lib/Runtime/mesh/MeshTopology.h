...
public:
  ...
  public:
    ...
    void resize(IdVec& v){
      ...
      groupVec_[n] = size;
      idVec_.resize(size);
      fill(idVec_.begin(), idVec_.end(), 0);
    }
  ...
  public:
    ...
    Coordinate(std::initializer_list<Float> il){
      static_assert(il.size() == MT::geometricDimension(), "coordinate size mismatch");
  ...
  public:
    ...
    Connectivity& getConnectivity(size_t fromDim, size_t toDim){
      assert(fromDim < connections_.size() && "invalid fromDim");
      auto& c = connections_[fromDim];
      assert(toDim < c.size() && "invalid toDim");
      return c[toDim];
    }
    
    void dump(){
      for(size_t i = 0; i < connections_.size(); ++i){
        auto& ci = connections_[i];
  ...
  void addCell(Id id, std::initializer_list<Id> il){
    ...
    auto& c = topology_.getConnectivity(MT::topologicalDimension(), 0);
    for(Id id : il){
      c.push(id);
    }
    c.endGroup();
  ...
  void build(size_t dim){
    ...
    Connectivity& entityToVertex = topology_.getConnectivity(dim, 0);
    
    Connectivity& cellToEntity =
    topology_.getConnectivity(MT::topologicalDimension(), dim);
  ...
  void transpose(size_t fromDim, size_t toDim){
    Connectivity& conn = topology_.getConnectivity(fromDim, toDim);
    
    Connectivity& fromConn = topology_.getConnectivity(toDim, fromDim);
  ...
  Connectivity& getConnectivity(size_t fromDim, size_t toDim){
    return topology_.getConnectivity(fromDim, toDim);
  }
...
private:
  std::array<size_t, MT::topologicalDimension() + 1> size_;
  Topology topology_;
  Geometry geometry_;
};
