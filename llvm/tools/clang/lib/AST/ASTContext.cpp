...
#include "clang/AST/TypeNodes.def"
  ...
  case Type::UnstructuredMesh:
  {
    const Type *L = LHS.getTypePtr();
    const Type *R = RHS.getTypePtr();

    if(L->isMeshType() && R->isMeshType()) {
      if(CompareMeshTypes(L, R)) {
        unsigned Lrank = L->getAs<MeshType>()->rankOf();
        unsigned Rrank = R->getAs<MeshType>()->rankOf();
        // if one side does not have rank set return the other
        // this is a mesh passed by parameter
        if (Lrank == 0) return RHS;
        if (Rrank == 0) return LHS;
        // if ranks match then also ok.
        if (Lrank == Rrank) return LHS;
      }
    }
    return QualType();
  }
