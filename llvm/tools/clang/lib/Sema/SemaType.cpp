...
QualType Sema::BuildArrayType(QualType T, ArrayType::ArraySizeModifier ASM,
  ...
  if (T->isVariableArrayType()) {
    ...
    // +==== Scout ===============================================================+
    // SC_TODO: this causes a error in scout mode for some unknown reason
...
QualType Sema::BuildFrameVarType(QualType T, const Type* elementType) {
  const FrameVarType* qt = dyn_cast<FrameVarType>(T.getCanonicalType().getTypePtr());
  if (qt) {
    return Context.getFrameVarType(elementType);
  }
  return QualType();
}
...
static TypeSourceInfo *GetFullTypeForDeclarator(TypeProcessingState &state,
  ...
  for (unsigned i = 0, e = D.getNumTypeObjects(); i != e; ++i) {
    ...
    case DeclaratorChunk::RectilinearMesh: {
      MeshType::MeshDimensions dims = DeclType.Rectmsh.Dims();
      T = S.BuildRectilinearMeshType(T, dims, SourceRange(DeclType.Loc,
                                     DeclType.EndLoc), Name);
      break;
    }
