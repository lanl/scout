...
// +===== Scout ==============================================================+
template<typename Derived>
bool DataRecursiveASTVisitor<Derived>::TraverseUniformMeshHelper(UniformMeshDecl *D) {
  // We shouldn't traverse D->getTypeForDecl(); it's a result of
  // declaring the type, not something that was written in the
  // source.
  TRY_TO(TraverseNestedNameSpecifierLoc(D->getQualifierLoc()));
...
template<typename Derived>
bool DataRecursiveASTVisitor<Derived>::TraverseStructuredMeshHelper(StructuredMeshDecl *D) {
  // We shouldn't traverse D->getTypeForDecl(); it's a result of
  // declaring the type, not something that was written in the
  // source.
  TRY_TO(TraverseNestedNameSpecifierLoc(D->getQualifierLoc()));
...
template<typename Derived>
bool DataRecursiveASTVisitor<Derived>::TraverseUnstructuredMeshHelper(UnstructuredMeshDecl *D) {
  // We shouldn't traverse D->getTypeForDecl(); it's a result of
  // declaring the type, not something that was written in the
  // source.
  TRY_TO(TraverseNestedNameSpecifierLoc(D->getQualifierLoc()));
...
DEF_TRAVERSE_STMT(StencilShiftExpr, {})
// ===========================================
