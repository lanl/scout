...
#ifndef _SC_CUDA_DRIVER2_H_
#define _SC_CUDA_DRIVER2_H_

#include "Driver.h"

static int alignTo2(int x, int alignment) {
  int mask = (1 << alignment) - 1;
  return (x + mask) & ~mask;
}

class CudaDriver2: public Driver {
 public:
  struct Memcpy {
    llvm::Value *host;
    llvm::Value *device;
  ...
  CudaDriver2(llvm::Module &module, llvm::IRBuilder<> &builder, bool debug = false);
