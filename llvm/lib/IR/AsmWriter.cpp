...
// +===== Scout ===========================================
  static void writeDIScoutDerivedType(raw_ostream &Out, const DIScoutDerivedType *N,
                                      TypePrinting *TypePrinter, SlotTracker *Machine,
                                      const Module *Context) {
    Out << "!DIScoutDerivedType(";
    MDFieldPrinter Printer(Out, TypePrinter, Machine, Context);
    Printer.printTag(N);
    Printer.printString("name", N->getName());
    Printer.printMetadata("scope", N->getRawScope());
    ...
    Printer.printDIFlags("flags", N->getFlags());
    Printer.printMetadata("extraData", N->getRawExtraData());
    Printer.printInt("scoutFlags", N->getScoutFlags());
    Out << ")";
  }
// +=======================================================
...
static void writeDIScoutCompositeType(raw_ostream &Out,
  ...
  Printer.printMetadata("templateParams", N->getRawTemplateParams());
  Printer.printString("identifier", N->getIdentifier());
  Printer.printInt("dimX", N->getDimX());
  Printer.printInt("dimY", N->getDimY());
  Printer.printInt("dimZ", N->getDimZ());
  Out << ")";
}
// +=======================================================
