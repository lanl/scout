...
inline __device__ void convert_normal(float3 & nf, float3 nf2) {
  //added by Xin
//  float invlen = rsqrtf(nf2.x*nf2.x + nf2.y*nf2.y + nf2.z*nf2.z);

//  nf = make_float3(nf2.x * invlen, nf2.y * invlen, nf2.z * invlen);
    nf = nf2;
}
...
classifyVoxel(uint2 * RESTRICT voxelVerts, 
              ...
              float isoValue, unsigned int* numVertsTable) {
    uint3 gridPos;
    unsigned int i;
...
#else
    numVerts.x = numVertsTable[cubeindex];
#endif
...
generateTriangleVerticesSMEM(float3 * RESTRICT pos, 
                             ...
                             int* triTable, unsigned int* numVertsTable) {
...
#else
    unsigned int numVerts = numVertsTable[cubeindex];
#endif
...
#else
        edge = triTable[(cubeindex*16) + i];//tex1Dfetch(triTex, (cubeindex*16) + i);
        vert[0] = &vertlist[(edge*NTHREADS)+threadIdx.x];

        edge = triTable[(cubeindex*16) + i + 1];//tex1Dfetch(triTex, (cubeindex*16) + i + 1);
        vert[1] = &vertlist[(edge*NTHREADS)+threadIdx.x];

        edge = triTable[(cubeindex*16) + i + 2];//tex1Dfetch(triTex, (cubeindex*16) + i + 2);
        vert[2] = &vertlist[(edge*NTHREADS)+threadIdx.x];
...
generateTriangleNormals(const float3 * RESTRICT pos,
                        ...
                        float3 bBoxInv, unsigned int numVerts, cudaTextureObject_t textureobj) {
  unsigned int zOffset = blockIdx.z * gridDim.x * gridDim.y;
  unsigned int blockId = (blockIdx.y * gridDim.x) + blockIdx.x;
  unsigned int i = zOffset + (blockId * blockDim.x) + threadIdx.x;
...
#else
  n.x = tex3D<float>(textureobj, p2.x, p2.y, p2.z) - tex3D<float>(textureobj, p1.x, p1.y, p1.z);
#endif
...
#else
  n.y = tex3D<float>(textureobj, p2.x, p2.y, p2.z) - tex3D<float>(textureobj, p1.x, p1.y, p1.z);
#endif
...
#else
  n.z = tex3D<float>(textureobj, p2.x, p2.y, p2.z) - tex3D<float>(textureobj, p1.x, p1.y, p1.z);
#endif
...
void ThrustScanWrapperUint2(uint2* output, uint2* input, unsigned int numElements) {
    ...
    thrust::host_vector<uint2> h_input(numElements);
    thrust::copy(thrust::device_ptr<uint2>(input),
                 thrust::device_ptr<uint2>(input + numElements),
                 h_input.begin());

    thrust::exclusive_scan(h_input.begin(),
                           h_input.end(),
                           h_input.begin(),
                           zero);

    thrust::copy(h_input.begin(),
                 h_input.end(),
...
void ThrustScanWrapperArea(float* output, float* input, unsigned int numElements) {
    thrust::host_vector<float> h_input(numElements);
    thrust::copy(thrust::device_ptr<float>(input),
                 thrust::device_ptr<float>(input + numElements),
                 h_input.begin());

    thrust::inclusive_scan(h_input.begin(),
                           h_input.end(),
                           h_input.begin());
...
void CUDAMarchingCubes::computeIsosurfaceVerts(float3* vertOut, unsigned int maxverts, dim3 & grid3) {
    ...
    if (cudacomputemajor >= 2) {
      ...
      if (useSubGrid) {
        classifyVoxel<1,1><<<grid3D, threads3D>>>(d_voxelVerts, d_volume, 
                             ...
                             subGridStart, subGridEnd, isoValue, d_numVertsTable);
      } else {
    ...
    generateTriangleVerticesSMEM<<<grid2, NTHREADS>>>(vertOut, 
        ...
        isoValue, activeVoxels, maxverts - 3, d_triTable, d_numVertsTable);
}
...
void CUDAMarchingCubes::computeIsosurface(float3* vertOut, float3* normOut, float3* colOut, unsigned int maxverts) {
    ...
    if (this->useColor) {
      switch (texformat) {
        case RGB3F:
          generateTriangleColorNormal<float3, float3, float3><<<grid3, threads>>>(vertOut, colOut, normOut, (float3 *) d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts, texObj);
          break;

        case RGB4U:
          generateTriangleColorNormal<float3, uchar4, float3><<<grid3, threads>>>(vertOut, colOut, normOut, (uchar4 *) d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts, texObj);
          break;
      }
    } else {
      generateTriangleNormals<float3><<<grid3, threads>>>(vertOut, normOut, gridSizeInv, bBoxInv, totalVerts, texObj);
    }
...
void CUDAMarchingCubes::computeIsosurface(float3* vertOut, float3* normOut, uchar4* colOut, unsigned int maxverts) {
    ...
    if (this->useColor) {
      switch (texformat) {
        case RGB3F:
          generateTriangleColorNormal<uchar4, float3, float3><<<grid3, threads>>>(vertOut, colOut, normOut, (float3 *) d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts, texObj);
          break;

        case RGB4U:
          generateTriangleColorNormal<uchar4, uchar4, float3><<<grid3, threads>>>(vertOut, colOut, normOut, (uchar4 *) d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts, texObj);
          break;
      }
    } else {
      generateTriangleNormals<float3><<<grid3, threads>>>(vertOut, normOut, gridSizeInv, bBoxInv, totalVerts, texObj);
...
void CUDAMarchingCubes::computeIsosurface(float3* vertOut, char3* normOut, uchar4* colOut, unsigned int maxverts) {
    ...
    if (this->useColor) {
      switch (texformat) {
        case RGB3F:
          generateTriangleColorNormal<uchar4, float3, char3><<<grid3, threads>>>(vertOut, colOut, normOut, (float3 *) d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts, texObj);
          break;

        case RGB4U:
          generateTriangleColorNormal<uchar4, uchar4, char3><<<grid3, threads>>>(vertOut, colOut, normOut, (uchar4 *) d_colors, this->gridSize, gridSizeInv, bBoxInv, totalVerts, texObj);
          break;
      }
    } else {
      generateTriangleNormals<char3><<<grid3, threads>>>(vertOut, normOut, gridSizeInv, bBoxInv, totalVerts, texObj);
    }
...
bool CUDAMarchingCubes::SetVolumeData(float *volume, cudaTextureObject_t cto, 
                        float3 *colors,
                       uint3 gridsize, float3 gridOrigin, 
                       float3 boundingBox, bool cudaArray)
{
  texObj = cto;
  SetVolumeData(volume, colors, gridsize, gridOrigin, boundingBox, cudaArray);
}
