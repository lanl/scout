...
# Version tweaked for use with Scout...
...
#   MPI_<lang>_LINK_LIBS       All libraries to link MPI programs against w/o semicolons
  ...
  if (interrogate AND ((NOT MPI_${lang}_INCLUDE_PATH) OR (NOT MPI_${lang}_LIBRARIES)))
    if (MPI_${lang}_COMPILER)
      ...
      if (MPI_COMPILE_CMDLINE OR MPI_LINK_CMDLINE)
        ...
        # Extract linker flags from the link command line
        string(REGEX MATCHALL "(^| )-L([^\" ]+|\"[^\"]+\")" MPI_ALL_LINK_DIRS "${MPI_LINK_CMDLINE}")
        set(MPI_LINK_DIRS_WORK)
        foreach(FLAG ${MPI_ALL_LINK_DIRS})
          if (MPI_LINK_DIRS_WORK)
            set(MPI_LINK_DIRS_WORK "${MPI_LINK_DIRS_WORK}${FLAG}")
          else()
            set(MPI_LINK_DIRS_WORK ${FLAG})
          endif()
        endforeach()

        # Extract the set of libraries to link against from the link command
        # line
#        string(REGEX MATCHALL "(^| )-l([^\" ]+|\"[^\"]+\")" MPI_LIBNAME_LIST "${MPI_LINK_CMDLINE}")
        string(REGEX MATCHALL "-l([^\" ]+|\"[^\"]+\")" MPI_LIBNAME_LIST "${MPI_LINK_CMDLINE}")
        set(MPI_LINK_LIBS_WORK "")
        foreach(arg ${MPI_LIBNAME_LIST})
          if (MPI_LINK_LIBS_WORK)
            set(MPI_LINK_LIBS_WORK "${arg} ${MPI_LINK_LIBS_WORK}")
          else()
            set(MPI_LINK_LIBS_WORK ${arg})
          endif()
        endforeach()

        set(MPI_LIBNAMES ${MPI_LIBNAME_LIST})
        unset(MPI_LIBNAME_LIST)
    ...
    set(MPI_${lang}_LINK_LIBS     ${MPI_LINK_LIBS_WORK}     CACHE STRING "MPI ${lang} implicit link libraries"   FORCE)
    set(MPI_${lang}_LINK_DIRS     ${MPI_LINK_DIRS_WORK}     CACHE STRING "MPI ${lang} link directories"          FORCE)
    mark_as_advanced(
      MPI_${lang}_COMPILE_FLAGS 
      MPI_${lang}_INCLUDE_PATH 
      MPI_${lang}_LINK_FLAGS 
      MPI_${lang}_LIBRARIES
      MPI_${lang}_LINK_LIBS
      MPI_${lang}_LINK_DIRS
      )
